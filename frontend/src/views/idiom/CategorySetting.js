...
import { addClassify, deleteClassify} from "../../redux/actions/idiom";
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        deleteClassify: id => dispatch(deleteClassify(id)),
    };
}
...
class CategorySetting extends Component {
    ...
    deleteItem = id => {    
        let confirmDelete = window.confirm('Delete this Classify?')
        if(confirmDelete){
          this.props.deleteClassify(id);
        }
    }
        ...
        const data = this.props.classifyList.map(item => {
            ...
            return (
                <tr key={item.id}>
                    ...
                    <td >
                        <div>
                            <Button variant="secondary" onClick={() => this.deleteItem(item.id)} className="btn btn-sm btn-outline-success pull-xs-right" style={{marginLeft:"10px"}}><i className="ion-ios-trash"></i></Button>
