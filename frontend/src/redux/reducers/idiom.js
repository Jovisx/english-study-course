/*  
* Written by Romy, 2019-11-08 
* Updated by Romy, 2019-11-13
*/

import { LOADED_IDIOMS, LOADED_CLASSIFYLIST, ADDED_CLASSIFY, DELETED_CLASSIFY, ADDED_IDIOM, UPDATED_IDIOM, DELETED_IDIOM, CHANGE_CATEGORY } from "../../constants";
...
function rootReducer(state = {totalIdioms: [], idioms:[], classifyList:[], currentId: 1}, action) {
    ...
    if (action.type === DELETED_CLASSIFY) {
        const temp = state.classifyList.filter(classify => classify.id !== action.id);
        return {
            ...state,
            classifyList: temp
        };
